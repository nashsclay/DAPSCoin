...
static bool AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || mapArgs.count("-?") || mapArgs.count("-help")) {
        ...
        std::string strUsage = _("PRCY prcycoin-tx utility version") + " " + FormatFullVersion() + "\n\n" +
                               _("Usage:") + "\n" +
...
static void RegisterLoad(const std::string& strInput)
{
    ...
    if (totalLength > MAX_FILE_LENGTH) {
        std::string strErr = "Error reading big file " + filename;
        throw std::runtime_error(strErr);
    }
...
static std::string readStdin()
{
    ...
    if (totalLength > MAX_FILE_LENGTH)
        throw std::runtime_error("error reading stdin max length");
