...
void CBudgetManager::CheckOrphanVotes()
{
    ...
    LogPrint(BCLog::MASTERNODE,"CBudgetManager::CheckOrphanVotes - Done\n");
}
...
void CBudgetManager::SubmitFinalBudget()
{
    ...
    if (nSubmittedHeight >= nBlockStart){
        LogPrint(BCLog::MASTERNODE,"CBudgetManager::SubmitFinalBudget - nSubmittedHeight(=%ld) < nBlockStart(=%ld) condition not fulfilled.\n", nSubmittedHeight, nBlockStart);
        return;
    }
    ...
    if (nBlockStart - nCurrentHeight > ((GetBudgetPaymentCycleBlocks() / 30) * 2)){
        LogPrint(BCLog::MASTERNODE,"CBudgetManager::SubmitFinalBudget - Too early for finalization. Current block is %ld, next Superblock is %ld.\n", nCurrentHeight, nBlockStart);
        LogPrint(BCLog::MASTERNODE,"CBudgetManager::SubmitFinalBudget - First possible block for finalization: %ld. Last possible block for finalization: %ld. You have to wait for %ld block(s) until Budget finalization will be possible\n", nFinalizationStart, nBlockStart, nOffsetToStart);
    ...
    if (vecTxBudgetPayments.size() < 1) {
        LogPrint(BCLog::MASTERNODE,"CBudgetManager::SubmitFinalBudget - Found No Proposals For Period\n");
        return;
    }
    ...
    if (mapSeenFinalizedBudgets.count(tempBudget.GetHash())) {
        LogPrint(BCLog::MASTERNODE,"CBudgetManager::SubmitFinalBudget - Budget already exists - %s\n", tempBudget.GetHash().ToString());
    ...
    if (!mapCollateralTxids.count(tempBudget.GetHash())) {
        ...
        if (!pwalletMain->GetBudgetSystemCollateralTX(wtx, tempBudget.GetHash(), false)) {
            LogPrint(BCLog::MASTERNODE,"CBudgetManager::SubmitFinalBudget - Can't make collateral transaction\n");
            return;
        }
    ...
    if (!GetTransaction(txidCollateral, txCollateral, nBlockHash, true)) {
        LogPrint(BCLog::MASTERNODE,"CBudgetManager::SubmitFinalBudget - Can't find collateral tx %s", txidCollateral.ToString());
        return;
    }
    ...
    */
    if (conf < Params().Budget_Fee_Confirmations() + 1) {
        LogPrint(BCLog::MASTERNODE,"CBudgetManager::SubmitFinalBudget - Collateral requires at least %d confirmations - %s - %d confirmations\n", Params().Budget_Fee_Confirmations() + 1, txidCollateral.ToString(), conf);
        return;
    }
    ...
    if (!finalizedBudgetBroadcast.IsValid(strError)) {
        LogPrint(BCLog::MASTERNODE,"CBudgetManager::SubmitFinalBudget - Invalid finalized budget - %s \n", strError);
        return;
    }
    ...
    nSubmittedHeight = nCurrentHeight;
    LogPrint(BCLog::MASTERNODE,"CBudgetManager::SubmitFinalBudget - Done! %s\n", finalizedBudgetBroadcast.GetHash().ToString());
}
...
bool CBudgetDB::Write(const CBudgetManager& objToSave)
{
    ...
    LogPrint(BCLog::MASTERNODE,"Written info to budget.dat  %dms\n", GetTimeMillis() - nStart);
...
CBudgetDB::ReadResult CBudgetDB::Read(CBudgetManager& objToLoad, bool fDryRun)
{
    ...
    LogPrint(BCLog::MASTERNODE,"Loaded info from budget.dat  %dms\n", GetTimeMillis() - nStart);
    LogPrint(BCLog::MASTERNODE,"  %s\n", objToLoad.ToString());
    if (!fDryRun) {
        LogPrint(BCLog::MASTERNODE,"Budget manager - cleaning....\n");
        objToLoad.CheckAndRemove();
        LogPrint(BCLog::MASTERNODE,"Budget manager - result:\n");
        LogPrint(BCLog::MASTERNODE,"  %s\n", objToLoad.ToString());
    }
...
void DumpBudgets()
{
    ...
    LogPrint(BCLog::MASTERNODE,"Verifying budget.dat format...\n");
    CBudgetDB::ReadResult readResult = budgetdb.Read(tempBudget, true);
    // there was an error and it was not an error on file opening => do not proceed
    if (readResult == CBudgetDB::FileError)
        LogPrint(BCLog::MASTERNODE,"Missing budgets file - budget.dat, will try to recreate\n");
    else if (readResult != CBudgetDB::Ok) {
        LogPrint(BCLog::MASTERNODE,"Error reading budget.dat: ");
    ...
    LogPrint(BCLog::MASTERNODE,"Writting info to budget.dat...\n");
...
bool CBudgetManager::AddProposal(CBudgetProposal& budgetProposal)
{
    ...
    LogPrint(BCLog::MASTERNODE,"CBudgetManager::AddProposal - proposal %s added\n", budgetProposal.GetName ().c_str ());
    return true;
}
...
void CBudgetManager::CheckAndRemove()
{
    ...
    while (it != mapFinalizedBudgets.end()) {
        ...
        if (!strError.empty ()) {
            LogPrint(BCLog::MASTERNODE,"CBudgetManager::CheckAndRemove - Invalid finalized budget: %s\n", strError);
        }
        else {
            LogPrint(BCLog::MASTERNODE,"CBudgetManager::CheckAndRemove - Found valid finalized budget: %s %s\n",
    ...
    while (it2 != mapProposals.end()) {
        ...
        if (!strError.empty ()) {
            LogPrint(BCLog::MASTERNODE,"CBudgetManager::CheckAndRemove - Invalid budget proposal - %s\n", strError);
            strError = "";
        }
        else {
             LogPrint(BCLog::MASTERNODE,"CBudgetManager::CheckAndRemove - Found valid budget proposal: %s %s\n",
...
bool CBudgetManager::FillBlockPayee(CMutableTransaction& txNew, CAmount nFees, bool fProofOfStake)
{
    ...
    if (fProofOfStake) {
        if (nHighestCount > 0) {
            ...
            LogPrint(BCLog::MASTERNODE,"CBudgetManager::FillBlockPayee - Budget payment to %s for %lld, nHighestCount = %d\n", address2.ToString(), nAmount, nHighestCount);
        }
        else {
            LogPrint(BCLog::MASTERNODE,"CBudgetManager::FillBlockPayee - No Budget payment, nHighestCount = %d\n", nHighestCount);
        }
    } else {
        ...
        if (nHighestCount > 0) {
            ...
            LogPrint(BCLog::MASTERNODE,"CBudgetManager::FillBlockPayee - Budget payment to %s for %lld\n", address2.ToString(), nAmount);
        }
    }
    return true;
}
...
bool CBudgetManager::IsBudgetPaymentBlock(int nBlockHeight)
{
    ...
    LogPrint(BCLog::MASTERNODE,"CBudgetManager::IsBudgetPaymentBlock() - nHighestCount: %lli, 5%% of Masternodes: %lli. Number of budgets: %lli\n", 
...
bool CBudgetManager::IsTransactionValid(const CTransaction& txNew, int nBlockHeight)
{
    ...
    LogPrint(BCLog::MASTERNODE,"CBudgetManager::IsTransactionValid() - nHighestCount: %lli, 5%% of Masternodes: %lli mapFinalizedBudgets.size(): %ld\n", 
              nHighestCount, nFivePercent, mapFinalizedBudgets.size());
    /*
        If budget doesn't have 5% of the network votes, then we should pay a masternode instead
    */
...
std::vector<CBudgetProposal*> CBudgetManager::GetBudget()
{
    ...
    while (it2 != vBudgetPorposalsSort.end()) {
        ...
        LogPrint(BCLog::MASTERNODE,"CBudgetManager::GetBudget() - Processing Budget %s\n", pbudgetProposal->strProposalName.c_str());
        ...
        if (pbudgetProposal->fValid && pbudgetProposal->nBlockStart <= nBlockStart &&
            ...
            LogPrint(BCLog::MASTERNODE,"CBudgetManager::GetBudget() -   Check 1 passed: valid=%d | %ld <= %ld | %ld >= %ld | Yeas=%d Nays=%d Count=%d | established=%d\n",
            ...
            if (pbudgetProposal->GetAmount() + nBudgetAllocated <= nTotalBudget) {
                ...
                LogPrint(BCLog::MASTERNODE,"CBudgetManager::GetBudget() -     Check 2 passed: Budget added\n");
            } else {
                pbudgetProposal->SetAllotted(0);
                LogPrint(BCLog::MASTERNODE,"CBudgetManager::GetBudget() -     Check 2 failed: no amount allotted\n");
            }
        }
        else {
            LogPrint(BCLog::MASTERNODE,"CBudgetManager::GetBudget() -   Check 1 failed: valid=%d | %ld <= %ld | %ld >= %ld | Yeas=%d Nays=%d Count=%d | established=%d\n",
                      pbudgetProposal->fValid, pbudgetProposal->nBlockStart, nBlockStart, pbudgetProposal->nBlockEnd,
                      nBlockEnd, pbudgetProposal->GetYeas(), pbudgetProposal->GetNays(), mnodeman.CountEnabled(ActiveProtocol()) / 10,
                      pbudgetProposal->IsEstablished());
        }
    ...
    while (it != mapFinalizedBudgets.end()) {
        ...
        if (nBlockHeight >= pfinalizedBudget->GetBlockStart() && nBlockHeight <= pfinalizedBudget->GetBlockEnd()) {
            ...
            if (pfinalizedBudget->GetBudgetPaymentByBlock(nBlockHeight, payment)) {
                ...
                LogPrint(BCLog::MASTERNODE,"CBudgetManager::GetRequiredPaymentsString - Couldn't find budget payment for block %d\n", nBlockHeight);
            }
        }
...
void CBudgetManager::NewBlock()
{
    ...
    LogPrint(BCLog::MASTERNODE,"CBudgetManager::NewBlock - mapFinalizedBudgets cleanup - size: %d\n", mapFinalizedBudgets.size());
    ...
    LogPrint(BCLog::MASTERNODE,"CBudgetManager::NewBlock - vecImmatureFinalizedBudgets cleanup - size: %d\n", vecImmatureFinalizedBudgets.size());
...
void CBudgetManager::ProcessMessage(CNode* pfrom, std::string& strCommand, CDataStream& vRecv)
{
    ...
    if (strCommand == NetMsgType::BUDGETVOTE) { // Budget Vote
        ...
        LogPrint(BCLog::MASTERNODE,"mvote - new budget vote for budget %s - %s\n", vote.nProposalHash.ToString(),  vote.GetHash().ToString());
    }
...
bool CBudgetManager::UpdateProposal(CBudgetVote& vote, CNode* pfrom, std::string& strError)
{
    ...
    if (!mapProposals.count(vote.nProposalHash)) {
        if (pfrom) {
            ...
            LogPrint(BCLog::MASTERNODE,"CBudgetManager::UpdateProposal - Unknown proposal %d, asking for source proposal\n", vote.nProposalHash.ToString());
...
bool CBudgetManager::UpdateFinalizedBudget(CFinalizedBudgetVote& vote, CNode* pfrom, std::string& strError)
{
    ...
    if (!mapFinalizedBudgets.count(vote.nBudgetHash)) {
        if (pfrom) {
            ...
            LogPrint(BCLog::MASTERNODE,"CBudgetManager::UpdateFinalizedBudget - Unknown Finalized Proposal %s, asking for source budget\n", vote.nBudgetHash.ToString());
    ...
    LogPrint(BCLog::MASTERNODE,"CBudgetManager::UpdateFinalizedBudget - Finalized Proposal %s added\n", vote.nBudgetHash.ToString());
...
bool CBudgetVote::Sign(CKey& keyMasternode, CPubKey& pubKeyMasternode)
{
    ...
    if (!CMessageSigner::VerifyMessage(pubKeyMasternode, vchSig, strMessage, strError)) {
        LogPrint(BCLog::MASTERNODE,"CBudgetVote::Sign - Error upon calling VerifyMessage: %s", strError);
        return false;
    }
...
bool CBudgetVote::SignatureValid(bool fSignatureCheck)
{
    ...
    if (!CMessageSigner::VerifyMessage(pmn->pubKeyMasternode, vchSig, strMessage, strError)) {
        LogPrint(BCLog::MASTERNODE,"CBudgetVote::SignatureValid() - Verify message failed, error: %s\n", strError);
        return false;
    }
...
void CFinalizedBudget::AutoCheck()
{
    ...
    LogPrint(BCLog::MASTERNODE,"CFinalizedBudget::AutoCheck - %lli - %d\n", pindexPrev->nHeight, fAutoChecked);
    ...
    if (Params().NetworkID() == CBaseChainParams::MAIN && secp256k1_rand32() % 4 != 0) {
        LogPrint(BCLog::MASTERNODE,"CFinalizedBudget::AutoCheck - waiting\n");
        return;
    }
    ...
    if (strBudgetMode == "auto") //only vote for exact matches
    {
        ...
        if (vBudgetProposals.size() == 0) {
            LogPrint(BCLog::MASTERNODE,"CFinalizedBudget::AutoCheck - Can't get Budget, aborting\n");
            return;
        }

        if (vBudgetProposals.size() != vecBudgetPayments.size()) {
            LogPrint(BCLog::MASTERNODE,"CFinalizedBudget::AutoCheck - Budget length doesn't match. vBudgetProposals.size()=%ld != vecBudgetPayments.size()=%ld\n",
        ...
        for (unsigned int i = 0; i < vecBudgetPayments.size(); i++) {
            if (i > vBudgetProposals.size() - 1) {
                LogPrint(BCLog::MASTERNODE,"CFinalizedBudget::AutoCheck - Proposal size mismatch, i=%d > (vBudgetProposals.size() - 1)=%d\n", i, vBudgetProposals.size() - 1);
                return;
            }

            if (vecBudgetPayments[i].nProposalHash != vBudgetProposals[i]->GetHash()) {
                LogPrint(BCLog::MASTERNODE,"CFinalizedBudget::AutoCheck - item #%d doesn't match %s %s\n", i, vecBudgetPayments[i].nProposalHash.ToString(), vBudgetProposals[i]->GetHash().ToString());
                return;
            }
            ...
            if (vecBudgetPayments[i].nAmount != vBudgetProposals[i]->GetAmount()) {
                LogPrint(BCLog::MASTERNODE,"CFinalizedBudget::AutoCheck - item #%d payee doesn't match %lli %lli\n", i, vecBudgetPayments[i].nAmount, vBudgetProposals[i]->GetAmount());
                return;
            }
        }

        LogPrint(BCLog::MASTERNODE,"CFinalizedBudget::AutoCheck - Finalized Budget Matches! Submitting Vote.\n");
        SubmitVote();
    }
}
// If masternode voted for a proposal, but is now invalid -- remove the vote
...
std::string CFinalizedBudget::GetStatus()
{
    ...
    for (int nBlockHeight = GetBlockStart(); nBlockHeight <= GetBlockEnd(); nBlockHeight++) {
        ...
        if (!GetBudgetPaymentByBlock(nBlockHeight, budgetPayment)) {
            LogPrint(BCLog::MASTERNODE,"CFinalizedBudget::GetStatus - Couldn't find budget payment for block %lld\n", nBlockHeight);
            continue;
        }
...
bool CFinalizedBudget::IsTransactionValid(const CTransaction& txNew, int nBlockHeight)
{
    ...
    if (nCurrentBudgetPayment < 0) {
        LogPrint(BCLog::MASTERNODE,"CFinalizedBudget::IsTransactionValid - Invalid block - height: %d start: %d\n", nBlockHeight, GetBlockStart());
        return false;
    }

    if (nCurrentBudgetPayment > (int)vecBudgetPayments.size() - 1) {
        LogPrint(BCLog::MASTERNODE,"CFinalizedBudget::IsTransactionValid - Invalid block - current budget payment: %d of %d\n", nCurrentBudgetPayment + 1, (int)vecBudgetPayments.size());
        return false;
    }
    ...
    for (CTxOut out : txNew.vout) {
        if (vecBudgetPayments[nCurrentBudgetPayment].payee == out.scriptPubKey && vecBudgetPayments[nCurrentBudgetPayment].nAmount == out.nValue) {
            found = true;
            LogPrint(BCLog::MASTERNODE,"CFinalizedBudget::IsTransactionValid - Found valid Budget Payment of %d for %d\n",
    ...
    if (!found) {
        ...
        LogPrint(BCLog::MASTERNODE,"CFinalizedBudget::IsTransactionValid - Missing required payment - %s: %d c: %d\n",
...
void CFinalizedBudget::SubmitVote()
{
    ...
    if (!CMessageSigner::GetKeysFromSecret(strMasterNodePrivKey, keyMasternode, pubKeyMasternode)) {
        LogPrint(BCLog::MASTERNODE,"CFinalizedBudget::SubmitVote - Error upon calling SetKey\n");
        return;
    }

    CFinalizedBudgetVote vote(activeMasternode.vin, GetHash());
    if (!vote.Sign(keyMasternode, pubKeyMasternode)) {
        LogPrint(BCLog::MASTERNODE,"CFinalizedBudget::SubmitVote - Failure to sign.");
        return;
    }

    if (budget.UpdateFinalizedBudget(vote, NULL, strError)) {
        LogPrint(BCLog::MASTERNODE,"CFinalizedBudget::SubmitVote  - new finalized budget vote - %s\n", vote.GetHash().ToString());

        budget.mapSeenFinalizedBudgetVotes.insert(std::make_pair(vote.GetHash(), vote));
        vote.Relay();
    } else {
        LogPrint(BCLog::MASTERNODE,"CFinalizedBudget::SubmitVote : Error submitting vote - %s\n", strError);
    }
}
...
bool CFinalizedBudgetVote::Sign(CKey& keyMasternode, CPubKey& pubKeyMasternode)
{
    ...
    if (!CMessageSigner::SignMessage(strMessage, vchSig, keyMasternode)) {
        LogPrint(BCLog::MASTERNODE,"CFinalizedBudgetVote::Sign - Error upon calling SignMessage");
        return false;
    }

    if (!CMessageSigner::VerifyMessage(pubKeyMasternode, vchSig, strMessage, strError)) {
        LogPrint(BCLog::MASTERNODE,"CFinalizedBudgetVote::Sign - Error upon calling VerifyMessage: %s", strError);
        return false;
    }
...
bool CFinalizedBudgetVote::SignatureValid(bool fSignatureCheck)
{
    ...
    if (pmn == NULL) {
        LogPrint(BCLog::MASTERNODE,"CFinalizedBudgetVote::SignatureValid() - Unknown Masternode %s\n", strMessage);
        return false;
    }
    ...
    if (!CMessageSigner::VerifyMessage(pmn->pubKeyMasternode, vchSig, strMessage, strError)) {
        LogPrint(BCLog::MASTERNODE,"CFinalizedBudgetVote::SignatureValid() - Verify message failed %s: %s\n", strMessage, strError);
        return false;
    }
