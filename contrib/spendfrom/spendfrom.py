...
def determine_db_dir():
    ...
    if platform.system() == "Darwin":
        ...
        return os.path.join(os.environ['APPDATA'], "PRCYcoin")
    return os.path.expanduser("~/.prcycoin")
...
def connect_JSON(config):
    """Connect to a prcycoin JSON-RPC server"""
...
def list_available(prcycoind):
    address_summary = dict()

    address_to_account = dict()
    for info in prcycoind.listreceivedbyaddress(0):
        address_to_account[info["address"]] = info["account"]

    unspent = prcycoind.listunspent(0)
...
def main():
    ...
    if options.amount is None:
        ...
        sanity_test_fee(prcycoind, txdata, amount*Decimal("0.01"))
        if options.dry_run:
            print(txdata)
        else:
