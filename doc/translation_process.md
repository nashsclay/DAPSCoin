...
The PRCYCoin project has been designed to support multiple localisations. This makes adding new phrases, and completely new languages easily achievable. For managing all application translations, PRCYCoin makes use of the Transifex online translation management tool.
...
See the [Transifex PRCYCoin project](https://www.transifex.com/prcycoin/prcycoin-translations/) to assist in translations.
...
You can find the PRCYCoin translation project at [https://www.transifex.com/prcycoin/prcycoin-translations/](https://www.transifex.com/prcycoin/prcycoin-translations/).
...
The Transifex PRCYCoin project config file is included as part of the repo. It can be found at `.tx/config`, however you shouldn’t need to change anything.
...
3. Update `src/qt/prcycoin_locale.qrc` manually or via
```bash
git ls-files src/qt/locale/*ts|xargs -n1 basename|sed 's/\(prcycoin_\(.*\)\).ts/<file alias="\2">locale\/\1.qm<\/file>/'
```
4. Update `src/Makefile.qt.include` manually or via
```bash
git ls-files src/qt/locale/*ts|xargs -n1 basename|sed 's/\(prcycoin_\(.*\)\).ts/  qt\/locale\/\1.ts \\/'
```
...
The PRCYCoin translation maintainers include *lyricidal*. You can find them, and others, in the [PRCYCoin Discord](https://discord.prcycoin.com).
