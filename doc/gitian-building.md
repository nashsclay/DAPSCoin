...
*Setup instructions for a gitian build of PRCYCoin using a VM or physical system.*
...
to the PRCYCoin GitHub release page.
...
- [Building PRCYCoin](#building-prcycoin-core)
...
```bash
gpg --output ${VERSION}-linux/${NAME}/prcycoin-linux-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-linux/$NAME/prcycoin-linux-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-osx-unsigned/$NAME/prcycoin-osx-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-osx-unsigned/$NAME/prcycoin-osx-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-win-unsigned/$NAME/prcycoin-win-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-win-unsigned/$NAME/prcycoin-win-${VERSION%\.*}-build.assert
```

Uploading Signatures
--------------------
Make a Pull Request (both the `.assert` and `.assert.sig` files) to the
[gitian.sigs](https://github.com/prcycoin/gitian.sigs/) repository:
...
You can also mail the files to Fuzzbawls (fuzzbawls@prcycoin.org) and he will commit them.

```bash
gpg --detach-sign ${VERSION}-linux/${NAME}/prcycoin-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${NAME}/prcycoin-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${NAME}/prcycoin-osx-*-build.assert
```
