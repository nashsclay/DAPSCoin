...
*Setup instructions for a gitian build of PRCY using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the PRCYcoin
...
- [Building PRCYcoin](#building-prcycoin)
...
Clone the git repositories for prcycoin and gitian and then checkout the prcycoin version that you want to build.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/prcycoin-crypto/prcycoin.git
cd prcycoin
...
Follow the instructions in [doc/release-process.md](release-process.md) in the prcycoin repository
...
To build PRCY (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the prcycoin repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/prcycoin/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/prcycoin-crypto/prcycoin
...
For example:
```bash
URL=https://github.com/crowning-/prcycoin.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit prcycoin=${COMMIT} --url prcycoin=${URL} ../prcycoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit prcycoin=${COMMIT} --url prcycoin=${URL} ../prcycoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit prcycoin=${COMMIT} --url prcycoin=${URL} ../prcycoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/prcycoin-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/prcycoin-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/prcycoin-build.assert
```
...
[prcycoin/gitian.sigs](https://github.com/prcycoin-crypto/gitian.sigs/) repository, or if that's not possible to create a pull
request.
